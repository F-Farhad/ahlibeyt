<article class="bg-white flex flex-col shadow my-4">
    <!-- Article Image -->
    <a href="{{route('category.show.post', [$category, $post])}}" class="hover:opacity-75">
        <img src="{{$post->getThumbnail()}}">
    </a>

    <div class="bg-white flex flex-col justify-start p-6">
        <a href="{{route('category.show.post', [$category, $post])}}" class="text-3xl font-bold hover:text-gray-700">
            {{$post->title}}
        </a>
        ...
        <a href="{{route('category.show.post', [$category, $post])}}" class="pb-6">
            {!! $post->short_content !!}
        </a>
